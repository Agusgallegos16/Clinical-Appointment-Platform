...
Generate **drag-and-drop websites** for Hostinger / any static host with the visual quality of award-winning studios (Active Theory, Locomotive, Resn, &Walsh).
...
## Context Framework code (REGLA OVERRIDE DE PROYECTO)
- Este proyecto usa **React + Vite + Vercel + Spring Boot**.
- IGNORAR las reglas de "No npm, no React, no build step".
- Usar los **Arquetipos (02-archetypes.md)**, **Efectos (03-effects-catalog.md)** y **Paletas** de la skill, pero implementándolos como **componentes React (.jsx)** y librerías de npm (ej. GSAP vía npm).
- La nómina de médicos y datos dinámicos deben consumirse del backend (`/api/public/doctores`) y NO de un manifest.js estático.
